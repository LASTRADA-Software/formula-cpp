...
Prose guides (MkDocs Material, `docs/`) and the generated API reference (Doxygen,
under `/api`) publish to GitHub Pages from `master` -- see
[`.github/workflows/pages.yml`](.github/workflows/pages.yml) -- at
<https://lastrada-software.github.io/formula-cpp/> once Pages is switched on for
this repository (Settings > Pages > Build and deployment > Source: GitHub
Actions; a repository setting, not something this workflow can turn on itself).
Build the guides locally with `mkdocs build --strict`; the gallery
(`docs/gallery.md`) is generated by `tools/gallery/main.cpp` and checked in, so
it reads on GitHub with nothing built at all.
