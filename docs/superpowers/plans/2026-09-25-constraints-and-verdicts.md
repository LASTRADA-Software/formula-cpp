...
- **One working tree, one writer — and a reviewer running mutations is a writer.** A "stale binary" was reported on the GCC/WSL leg during phase 9: a build printing `unchecked` where the source says `not checked`. It was not a stale binary and not a filesystem problem. The whole-branch reviewer was mutating `examples/constraints.cpp` in the shared tree at 06:29:53 while the task-6 implementer was rebuilding in WSL; the implementer compiled the reviewer's live mutation, the reviewer restored the file a minute later, and the "clean rebuild that fixed it" simply rebuilt against restored source. Every symptom followed from one concurrent edit. **Never leave a mutating reviewer running while dispatching a fix round, and never dispatch a whole-branch review against a branch that is still moving.** A concurrent edit is indistinguishable from a build-system fault while you are inside it, and it will be misdiagnosed as one. **The detector, since a rule that says what not to do without saying how to notice you have done it is half a rule:** a symptom involving text that appears in *no commit* is a concurrency symptom until proven otherwise. Check with `git grep` and `git log -S` for the mystery string before reaching for any explanation involving the build system, the filesystem, or the compiler. In the phase-9 incident the string was in hand the whole time and nobody asked where it came from.
